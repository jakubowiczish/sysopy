...
int main(int argc, char** argv) {
    if (argc != 3 && argc != 4) {
        print_coloured_message(
            "invalid arguments given - necessary arguments: workers, "
            "max_weight, "
            "(cycles)",
            RED);
    }
    ...
    for (int i = 0; i < workers; ++i) {
        ...
        switch (fork()) {
            case -1:
                ...
                break;
            case 0:
                execl("./loader", "./loader", weight_arr, cycles_arr,
                      (char*)NULL);
    ...
    for (int i = 0; i < workers; ++i) {
        ...
        sprintf(loader_finished_buffer,
                "loader %d finished with exit status: %d", i,
                WEXITSTATUS(status));
