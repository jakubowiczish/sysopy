    ...
    while (is_server_running) {
        ...
        if (mq_receive(server_queue, input, MAX_MESSAGE_SIZE, NULL) == -1) {
            ...
            sprintf(buffer,
                    ...
                    type_to_string(input_msg.msg_type),
                    input
            );
...
void prepare_message() {
    ...
    sprintf(tmp, "from %d, %s - %s", actual_usr_id, input_msg.msg_text.buf, date);
...
int send_message(int id, int type) {
    ...
    sprintf(output,
            "%ld|%d|%d|%s",
            output_msg.msg_type,
            output_msg.msg_text.id,
            output_msg.msg_text.additional_arg,
            output_msg.msg_text.buf
    );


    if (user_exists(id) && mq_send(clients_queue_id_arr[id], output, strlen(output) + 1, output_msg.msg_type) == -1) {
        ...
        for (int j = 0; j < groups_size_arr[i]; j++) {
            ...
            if (friends_groups_size_arr[i][j] == actual_usr_id) {
                ...
                for (int k = j; k < groups_size_arr[i] - 1; k++) {
                    friends_groups_size_arr[i][k] = friends_groups_size_arr[i][k + 1];
                }
...
void list_command() {
    ...
    for (int i = 0; i < next_client_id; i++) {
        ...
        memcpy(output_msg.msg_text.buf + offset, item, strlen(item) * sizeof(char));
...
void friends_command() {
    struct string_array id_list = process_file(input_msg.msg_text.buf, strlen(input_msg.msg_text.buf), ',');
    ...
    for (int i = 0; i < id_list.size; i++) {
        ...
        friends_groups_size_arr[actual_usr_id][groups_size] = strtol(id_list.data[i], NULL, 0);

        if (
                friends_groups_size_arr[actual_usr_id][groups_size] >= 0
                && user_exists(friends_groups_size_arr[actual_usr_id][groups_size])
                && !exists_in_group(actual_usr_id, friends_groups_size_arr[actual_usr_id][groups_size])
                && friends_groups_size_arr[actual_usr_id][groups_size] != actual_usr_id
                ) {
    ...
    if (groups_size == 0) {
        ...
        sprintf(output_msg.msg_text.buf, "FRIENDS - Create group with size %d.", groups_size_arr[actual_usr_id]);
    }
...
void add_command() {
    ...
    for (int i = 0; i < id_list.size; i++) {
        ...
        friends_groups_size_arr[actual_usr_id][group_size] = strtol(id_list.data[i], NULL, 0);

        if (
                friends_groups_size_arr[actual_usr_id][group_size] >= 0
                && user_exists(friends_groups_size_arr[actual_usr_id][group_size])
                && !exists_in_group(actual_usr_id, friends_groups_size_arr[actual_usr_id][group_size])
                && friends_groups_size_arr[actual_usr_id][group_size] != actual_usr_id
                ) {
    ...
    if (group_size == 0) {
        ...
        sprintf(output_msg.msg_text.buf, "ADD - Create group with size %d.", groups_size_arr[actual_usr_id]);
    }
...
void del_command() {
        ...
        for (int j = 0; j < group_index; j++) {
            ...
            if (friends_groups_size_arr[usr_id][j] == user_group_id) {
                ...
                for (int k = j; k < group_index - 1; k++) {
                    friends_groups_size_arr[usr_id][k] = friends_groups_size_arr[usr_id][k + 1];
                }
    ...
    if (group_index == 0) {
        ...
        sprintf(output_msg.msg_text.buf, "DEL - New group size is %d.", groups_size_arr[usr_id]);
    }
...
void _2all_command() {
    ...
    int sendCounter = 0;
    for (int i = 0; i < next_client_id; i++) {
        if (i != actual_usr_id && send_message(i, 1)) sendCounter++;
    }

    sprintf(output_msg.msg_text.buf, "2ALL - Send %d/%d message", sendCounter, next_client_id - 1);
}
...
void _2friends_command() {
    ...
    for (int i = 0; i < groups_size_arr[actual_usr_id]; i++) {
        if (
                friends_groups_size_arr[actual_usr_id][i] != actual_usr_id
    ...
    sprintf(output_msg.msg_text.buf, "2FRIENDS Send %d/%d message", sent_counter, groups_size_arr[actual_usr_id]);
}
