...
void friends_command(struct msg* input, struct msg* output) {
    ...
    struct string_array id_list =
        process_file(input->msg_text.buf, strlen(input->msg_text.buf), ',');
    ...
    for (int i = 0; i < id_list.size; i++) {
        friends_groups_arr[actual_usr_id][groups_size] =
            strtol(id_list.data[i], NULL, 0);

        if (friends_groups_arr[actual_usr_id][groups_size] >= 0 &&
            user_exists(friends_groups_arr[actual_usr_id][groups_size]) &&
            !exists_in_group(actual_usr_id,
                             friends_groups_arr[actual_usr_id][groups_size]) &&
    ...
    if (groups_size == 0) {
        ...
        sprintf(output->msg_text.buf, "FRIENDS - Create group with size %d.",
 ...
 * Wysłanie ADD lista_id_klientów po uprzednim wyczyszczeniu listy jest
 * analogiczne z wysłaniem FRIENDS lista_id_klientów. Próba wysłania ADD i DEL
 * bez argumentów powinna zostać obsłużona po stronie klienta.
 */
void add_command(struct msg* input, struct msg* output) {
    struct string_array id_list =
        process_file(input->msg_text.buf, strlen(input->msg_text.buf), ',');
    ...
    if (groups_size == 0) {
        ...
        sprintf(output->msg_text.buf, "ADD - Create group with size %d.",
                groups_size_arr[actual_usr_id]);
    }
...
void del_command(struct msg* input, struct msg* output) {
        ...
        for (int j = 0; j < group_index; j++) {
            if (friends_groups_arr[user_id][j] == user_group_id) {
                for (int k = j; k < group_index - 1; k++) {
                    friends_groups_arr[user_id][k] =
                        friends_groups_arr[user_id][k + 1];
                }
    ...
    if (group_index == 0) {
        ...
        sprintf(output->msg_text.buf, "DEL - New group size is %d.",
 ...
 * Klient wysyła ciąg znaków.
 * Serwer wysyła ten ciąg wraz z identyfikatorem klienta-nadawcy oraz aktualną
 * datą do zdefiniowanej wcześniej grupy klientów.
 */
void _2friends_command(struct msg* input, struct msg* output) {
    prepare_message(input, output);
    ...
    sprintf(output->msg_text.buf, "2FRIENDS Send %d/%d message", sent_counter,
            groups_size_arr[actual_usr_id]);
}
