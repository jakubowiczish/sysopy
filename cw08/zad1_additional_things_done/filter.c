...
int filter_load(const char* path, filter_t* filter) {
        ...
        if (main_line == 0) {
            ...
            while (ptr != NULL) {
                ...
                if (parse_double(ptr, &tmp) < 0) {
                    return err("Cannot open file (unknown image format.sh)",
                               -1);
                }
    ...
    if (index != (filter->size * filter->size))
        return err("Cannot open file (unknown image format.sh)", -1);
    ...
    for (int yp = y + left_offset; yp <= y + right_offset; yp++)
        for (int xp = x + left_offset; xp <= x + right_offset; xp++) {
            ...
            switch (image_in->color_mode) {
                case COLOR_BW:
                case COLOR_GRAY:
                    pixel_values[0] += filter->array[filter_index] *
                                       image_in->array[pixel_index];
                    break;

                case COLOR_RGB:
                    for (int i = 0; i < 3; i++)
                        pixel_values[i] += filter->array[filter_index] *
                                           image_in->array[pixel_index + i];
                    break;
