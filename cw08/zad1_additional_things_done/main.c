...
void timeval_diff(struct timeval *start, struct timeval *stop, struct timeval *diff) {
...
int main(int argc, char *argv[]) {
    ...
    if (threads_type != THREADS_TYPE_BLOCK && threads_type != THREADS_TYPE_INTERLEAVED)
    ...
    printf("Printing args %d %d %s %s %s\n", threads_num, threads_type, image_path, filter_path, out_image_path);
    ...
    for (int i = 0; i < threads_num; i++) {
        ...
        printf("Thread[%d] with id: %ld finished task in %lds %ldus\n",
               i, threads[i], thread_return->tv_sec, thread_return->tv_usec);
    ...
    printf("Program finished task in: %2lds %6ldus\n", timestamp_diff.tv_sec, timestamp_diff.tv_usec);
