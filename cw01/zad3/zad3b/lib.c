...
int search_directory(char* dir, char* file, char* name_file_temp) {
    ...
    if (snprintf(buffer, sizeof(buffer), "find %s -name %s > %s", searcher->dir,
                 searcher->file, searcher->name_file_temp) >= sizeof(buffer)) {
        printf("ERROR");
    } else {
        system(buffer);
    }

    //    printf("Searching the directory: %s, for: %s, results are stored in:
